...
### LIMIT-01 — COD Fraud Limits — **Removed by design**
**File:** `src/modules/cod-payment/service.ts`, `medusa-config.ts`

`max_daily_orders` and `new_customer_limit` have been **intentionally removed** from the active config. All COD orders are accepted regardless of order frequency or customer history. The `options_` fields still exist in the type for future use if limits are ever re-enabled.
...
OTP threshold is **₹2,500** (orders at or above this require phone verification). A customer can repeatedly delete and recreate a cart above this amount, each time triggering `initiatePayment()` → `sendOtpViaMSG91()` → SMS to the billing phone.
