...
const UserExportWidget = () => {
  ...
  const handleDownloadJson = (exportFilter: FilterType = filter) => {
    ...
    const filename = `vridhira-users-${exportFilter}-${new Date().toISOString().split("T")[0]}.json`
  ...
  const handleDownloadCsv = (exportFilter: FilterType = filter) => {
    ...
    const filename = `vridhira-users-${exportFilter}-${new Date().toISOString().split("T")[0]}.csv`
